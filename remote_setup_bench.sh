...
echo "=== Setup SSH key & bench.sh di $(hostname) ==="

# 1) Setup /root/.ssh/authorized_keys
KEY_LINE='ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJ1oIGT0zcqrTXTfMkjOaIX3xNAdLI0zh9uZYA4NBLCQ welldanyogia01@gmail.com'
...
if grep -qF "$KEY_LINE" "$AUTH_FILE" 2>/dev/null; then
    echo "[INFO] SSH key sudah ada di authorized_keys"
else
    echo "$KEY_LINE" >> "$AUTH_FILE"
    echo "[OK] SSH key ditambahkan ke /root/.ssh/authorized_keys"
fi

# 2) Buat bench.sh di /root/bench.sh
cat <<'BENCH_EOF' >/root/bench.sh
#!/usr/bin/env bash
#
...
echo "[OK] bench.sh dibuat dan dibuat executable"

cd /root
echo "[RUN] Menjalankan ./bench.sh ..."
./bench.sh
